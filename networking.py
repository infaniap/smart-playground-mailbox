"""
Original module developed in Tufts CEEO lab environment.(2025).
"""
...
class Networking:
    ...
    class Aen:
        ...
        def update_peer(self, peer_mac, peer_config=None, channel=None, ifidx=None):
            ...
            if peer_mac in self._peers:
                try:
                    ...
                    self.master.dprint(
                        f"Peer {peer_mac} updated to channel {channel}, ifidx {ifidx} and name {self.peer_name(peer_mac)}")
                except OSError as e:
        ...
        def add_peer(self, peer_mac, peer_config=None, channel=None, ifidx=None):
            ...
            if peer_mac not in self._peers:
                try:
                    ...
                    self.master.dprint(
                        f"Peer {peer_mac} added with channel {channel}, ifidx {ifidx} and name {self.peer_name(peer_mac)}")
                except OSError as e:
        ...
        def ping(self, mac, channel=None, ifidx=None):  # Ping
            ...
            self.send_custom(0x01, 0x10, mac, [send_channel, self.ifidx, self.master.config], channel,
        ...
        def send_data(self, mac, message, channel=None,
                      ifidx=None):  # message is a dict, key is the sensor type and the value is the sensor value, fix name!!!!!!!
        ...
        def _receive(self):
            ...
            def __process_message(self, sender_mac, message, receive_timestamp):
                ...
                checksum = message[-1]
                self.master.dprint(
                    f"{type(msg_type)}: {msg_type}, {type(subtype)}: {subtype}, {type(send_timestamp)}: {send_timestamp}, {type(payload_type)}: {payload_type},  {type(payload_bytes)}: {payload_bytes},  {type(checksum)}: {checksum}")
                ...
                if msg_type == 0x01:  # Command Message
                    msg_key = "cmd"
                    __handle_cmd(self, sender_mac, subtype, send_timestamp, receive_timestamp, payload, payload_size,
                                 msg_key)
                elif msg_type == 0x02:  # Informational Message
                    msg_key = "inf"
                    __handle_inf(self, sender_mac, subtype, send_timestamp, receive_timestamp, payload, payload_size,
                                 msg_key)
                elif msg_type == 0x03:  # Acknowledgement Message
                    msg_key = "ack"
                    __handle_ack(self, sender_mac, subtype, send_timestamp, receive_timestamp, payload, payload_size,
                                 msg_key)
                else:
                    self.master.iprint(
                        f"Unknown message type from {sender_mac} ({self.peer_name(sender_mac)}): {message}")

            def __handle_cmd(self, sender_mac, subtype, send_timestamp, receive_timestamp, payload, payload_size,
                             msg_key):
                ...
                msg_subkey := "RSSI/Status/Config-Boop") and subtype == 0x03 or subtype == 0x13:  # RSSI/Status/Config Boop
                    ...
                    try:
                        self._compose(sender_mac,
                                      [self.master.config, self.master.version, self.master.sta.mac, self.master.ap.mac,
                                       self.rssi()], 0x02, 0x20)  # [ID, Name, Config, Version, sta mac, ap mac, rssi]
                    except Exception as e:
                        self.master.aen.__send_confirmation(self, "Fail", sender_mac, f"{msg_subkey} ({subtype})",
                                                            payload, e)
                elif (msg_subkey := "Echo") and subtype == subtype == 0x02 or subtype == 0x15:  # Echo
                    self.master.iprint(
                        f"{msg_subkey} ({subtype}) command received from {sender_mac} ({self.peer_name(sender_mac)}): {payload}")  # Check i or d
            ...
            def __handle_inf(self, sender_mac, subtype, send_timestamp, receive_timestamp, payload, payload_size,
                             msg_key):
                self.master.dprint("aen.__handle_inf")
                if (
                msg_subkey := "RSSI/Status/Config-Boop") and subtype == 0x00 or subtype == 0x20:  # RSSI/Status/Config-Boop
                    self.master.iprint(
            ...
            def __handle_ack(self, sender_mac, subtype, send_timestamp, receive_timestamp, payload, payload_size,
                             msg_key):
                ...
                if (msg_subkey := "Pong") and subtype == 0x10:  # Pong
                    ...
                    self.master.iprint(
                        f"{msg_subkey} ({subtype}) received from {sender_mac} ({self.peer_name(sender_mac)}) for type {payload[0]} with error {payload[1]} and payload {payload[2]}")
                    # add to ack buffer
